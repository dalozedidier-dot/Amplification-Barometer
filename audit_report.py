...
def _build_verdict(
    *,
    ...
    l_perf_pro: Mapping[str, Any],
    targets: Mapping[str, Any],
) -> Dict[str, Any]:
    """Verdict multidimensionnel.

    Objectif: éviter un label unique et fournir des axes auditables.
    On reste volontairement simple et robuste: scores bornés [0,1] dérivés
    de métriques déjà présentes dans le rapport.
    """
    # Stabilité: moyenne des corrélations Spearman de stabilité (si dispo)
    stab_scores: list[float] = []
    for k, v in stability.items():
        if isinstance(k, str) and k.endswith("_spearman"):
            try:
                stab_scores.append(float(v))
            except Exception:
                pass
    stability_score = float(np.nanmean(stab_scores)) if stab_scores else float("nan")
    stability_score01 = _clamp01((stability_score + 1.0) / 2.0) if np.isfinite(stability_score) else 0.5

    # L_cap / L_act viennent de la maturity (déjà normalisés 0..1)
    cap01 = _clamp01(float(maturity.get("cap_score_enforced", maturity.get("cap_score_raw", 0.5))))
    act01 = _clamp01(float(maturity.get("act_score_enforced", maturity.get("act_score_raw", 0.5))))

    # Résilience: proxy via performance proactive (prevented_exceedance_rel)
    prevented = float(l_perf_pro.get("prevented_exceedance_rel", 0.0))
    resil01 = _clamp01(prevented / 0.20)  # 0.20 = "très bon" dans la démo

    # Gouvernance: target rule_execution_gap
    gov_ok = bool(targets.get("rule_execution_gap_meets_target", False))
    gov01 = 1.0 if gov_ok else 0.0

    dimensions = {
        "stability": {"score": stability_score01, "raw": stability_score},
        "L_cap": {"score": cap01},
        "L_act": {"score": act01},
        "resilience": {"score": resil01, "raw": prevented},
        "governance": {"score": gov01, "meets_target": gov_ok},
    }

    overall = float(np.mean([dimensions[d]["score"] for d in dimensions]))
    if overall >= 0.75:
        label = "Mature"
    elif overall >= 0.50:
        label = "Intermédiaire"
...
def build_audit_report(
    ...
    *,
    ...
    summary = {
        "P": _series_stats(p),
        "O": _series_stats(o),
        # E est un quasi-stock (cumul d'un flux). On expose un alias explicite.
        "E": _series_stats(e),
        "E_stock": _series_stats(e),
        # R est un niveau (marges, redondance, diversité, récupération). Alias explicite.
        "R": _series_stats(r),
    ...
    verdict = _build_verdict(stability=stability, maturity=maturity, l_perf_pro=l_perf_pro, targets=targets)
